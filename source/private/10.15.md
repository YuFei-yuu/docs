...
### 10.17-10.19：串口测试
测试代码：小电脑本地里有两个工作空间 24Sentry_AutoAim 24Sentry_Nav
<BR>
...
3. 24Sentry_Nav中：
    ``` shell
    ros2 run chassis_cmd twist2chassis_cmd_node
    ```
    ``` shell
    ros2 run chassis_cmd twist_transformer_node
    ```
4. 启动键盘控制
    ``` shell
